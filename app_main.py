...
if module == "🏠 Home":
    ...
    if budget_file:
        ...
        col1, col2, col3 = st.columns(3)
        col1.metric("Avg Monthly Income", f"Rs.{avg_inc:,.0f}")
        col2.metric("Avg Monthly Expenses", f"Rs.{avg_exp:,.0f}")
        col3.metric("Avg Net Surplus", f"Rs.{avg_net:,.0f}")
        ...
        if st.button("Generate Forecast", type="primary"):
            with st.spinner("Generating forecast..."):
                ...
                Avg income: Rs.{avg_inc:,.0f}
                Avg expenses: Rs.{avg_exp:,.0f}
                Avg net: Rs.{avg_net:,.0f}
